...
func DisplayLogs(logEntriesChan <-chan LogEntry, index int) {
	for logEntry := range logEntriesChan {
		DisplayLogEntry(logEntry, index)
	}
}

func DisplayLogEntry(logEntry LogEntry, index int) {
	line := logEntry.Msg
...
func getPrefixColor(index int, source string) lipgloss.AdaptiveColor {
	if index == STATIC_INDEX {
		// if source == string(LogSourceProvider) {
		// 	return common.Yellow
		// } else {
		// 	return common.Green
		// }
	}
	return common.LogPrefixColors[index%len(common.LogPrefixColors)]
}
